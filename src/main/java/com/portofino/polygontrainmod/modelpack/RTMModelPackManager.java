package com.portofino.polygontrainmod.modelpack;
...
/**
 * RTM-inspired ModelPackManager with advanced JSON processing and caching
 * Handles vehicle packs, textures, models, and configuration files
 */
public class RTMModelPackManager implements ResourceManagerReloadListener {
    public static final RTMModelPackManager INSTANCE = new RTMModelPackManager();
    
    // RTM-style file patterns
    ...
    public void initialize(ResourceManager resourceManager) {
        ...
        PolygonTrainMod.LOGGER.info("Initializing RTM Model Pack Manager...");
        ...
        try {
            ...
            initialized = true;
            PolygonTrainMod.LOGGER.info("RTM Model Pack Manager initialized successfully");
            
        } catch (Exception e) {
            PolygonTrainMod.LOGGER.error("Failed to initialize RTM Model Pack Manager", e);
        }
    }
    
    /**
     * Scan for vehicle configuration files (RTM-style)
     */
        ...
        try {
            ...
            for (Map.Entry<ResourceLocation, Resource> entry : resources.entrySet()) {
                try {
                    ...
                    PolygonTrainMod.LOGGER.warn("Failed to load vehicle config: {}", entry.getKey(), e);
                }
            }
        } catch (Exception e) {
            PolygonTrainMod.LOGGER.warn("Failed to scan vehicle configs", e);
        }
    }
    ...
    private void loadVehicleConfig(ResourceLocation location, Resource resource) throws IOException {
        try (InputStream is = resource.open();
            ...
            PolygonTrainMod.LOGGER.debug("Loaded vehicle config: {}", packId);
        }
    }
    ...
    /**
     * Parse resource configuration with RTM-style validation
     */
    ...
    private List<VehicleConfig> parseVehicleConfigs(com.google.gson.JsonArray vehiclesArray) {
        ...
        for (JsonElement element : vehiclesArray) {
            ...
            // Parse RTM-style seat positions
            vehicle.seatPositions = parseRTMSeatPositions(vehicleObj);
            
            // Parse RTM-style bogie positions
            vehicle.bogiePositions = parseRTMBogiePositions(vehicleObj);
            
            // Parse bogie models
            vehicle.bogieModels = parseRTMBogieModels(vehicleObj);
            
            // Parse textures
            vehicle.textures = parseRTMTextures(vehicleObj);
    ...
    /**
     * Parse RTM-style seat positions with multiple field support
     */
    private List<net.minecraft.world.phys.Vec3> parseRTMSeatPositions(JsonObject obj) {
        List<net.minecraft.world.phys.Vec3> seats = new ArrayList<>();

        // RTM style: seatPos is often 1/16 units, seatPosF/playerPos/playerPosF are block units.
        appendVec3Array(obj, "seatPos", seats, 1.0 / 16.0);
        appendVec3Array(obj, "seatPosF", seats, 1.0);
        appendVec3Array(obj, "playerPos", seats, 1.0);
        appendVec3Array(obj, "playerPosF", seats, 1.0);
     ...
     * Parse RTM-style bogie positions
     */
    private List<net.minecraft.world.phys.Vec3> parseRTMBogiePositions(JsonObject obj) {
        List<net.minecraft.world.phys.Vec3> bogies = new ArrayList<>();

        // These fields are expected to be in block units in RTM packs.
    ...
    /**
     * Parse RTM-style bogie models
     */
    private List<BogieModelConfig> parseRTMBogieModels(JsonObject obj) {
        List<BogieModelConfig> bogieModels = new ArrayList<>();
        
        // Check for RTM-style bogieModel3 array
        if (obj.has("bogieModel3") && obj.get("bogieModel3").isJsonArray()) {
            ...
            for (JsonElement element : array) {
                ...
                bogieConfig.textures = parseRTMTextures(bogieObj);
            ...
            if (bogieModel != null) {
                ...
                bogieConfig.textures = parseRTMTextures(bogieModel);
    ...
    /**
     * Parse RTM-style textures with array format
     */
    private Map<String, String> parseRTMTextures(JsonObject modelObj) {
    ...
    private void appendVec3Array(JsonObject obj, String key, List<net.minecraft.world.phys.Vec3> out, double scale) {
        ...
        for (JsonElement element : array) {
            ...
            try {
                ...
                PolygonTrainMod.LOGGER.warn("Failed to parse Vec3 from {}: {}", key, e.getMessage());
            }
        }
    }
    ...
    private void loadModelPacks(ResourceManager resourceManager) {
        for (String packId : resourceConfigs.keySet()) {
            ...
            try {
                ...
                PolygonTrainMod.LOGGER.debug("Loaded model pack: {}", packId);
            } catch (Exception e) {
                PolygonTrainMod.LOGGER.warn("Failed to load model pack: {}", packId, e);
            }
        }
    }
    ...
    private void validateAndRegisterResources() {
        for (ModelPack pack : loadedPacks.values()) {
            try {
                ...
                PolygonTrainMod.LOGGER.warn("Failed to validate/register pack: {}", pack.getPackId(), e);
    ...
    /**
     * Load script with include processing (RTM-style)
     */
    ...
    private String processScriptIncludes(String rawScript) {
        ...
        while (matcher.find()) {
            ...
            try {
                ...
                PolygonTrainMod.LOGGER.warn("Failed to include script: {}", includePath, e);
    ...
    private String loadScriptFile(String fileName) throws IOException {
        if (resourceManager == null) {
            throw new IOException("RTM model pack resource manager is not initialized");
        }
    ...
    public static class ModelPack {
        ...
        public void registerVehicles() {
            for (VehicleConfig vehicle : config.vehicles) {
                try {
                    // Convert RTM VehicleConfig to PolygonTrainMod VehicleDefinition
                    ...
                    PolygonTrainMod.LOGGER.warn("Failed to register vehicle: {}", vehicle.id, e);
                }
            }
        }
        
        private VehicleDefinition convertToVehicleDefinition(VehicleConfig vehicle) {
            // Convert RTM config to PolygonTrainMod format
            ...
            return new VehicleDefinition(
                ...
                java.util.List.of(), // playerPositions (RTMModelPackManager では未対応)
                vehicle.seatPositions.isEmpty() ? null : vehicle.seatPositions.get(0),
                scriptPath,
                vehicle.doorType,
                vehicle.trainDistance,
                0, // driverSeatIndex (RTMでは通常0番目が運転席)
    ...
    public static class VehicleConfig {
        ...
        public void validate() throws IOException {
            ...
            if (seatPositions.isEmpty() && bogiePositions.isEmpty()) {
                PolygonTrainMod.LOGGER.warn("Vehicle '{}' has no seats or bogies defined", id);
            }
        }
    }
